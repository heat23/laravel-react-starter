...
describe('Billing Index Page', () => {
  ...
  it('renders billing page for user without subscription', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Billing', level: 1 })
    ).toBeInTheDocument();
    expect(
      screen.getByText('Manage your subscription and payment details')
    ).toBeInTheDocument();
    expect(
      screen.getByText('Ready to unlock premium features?')
    ).toBeInTheDocument();
  ...
  it('renders incomplete payment alert', async () => {
    ...
    expect(
      screen.getByText('Payment Confirmation Required')
    ).toBeInTheDocument();
  ...
  it('renders past due subscription alert', async () => {
    ...
    expect(
      screen.getByText('Payment Failed - Automatic Retry in Progress')
    ).toBeInTheDocument();
  ...
  it('fires BILLING_TRIAL_UPGRADE_CLICKED when subscribe button clicked during trial (proPriceId present)', async () => {
    const { usePage } = await import('@inertiajs/react');
    vi.mocked(usePage).mockReturnValue({
      props: createMockProps({
        platformTrial: {
    ...
    const subscribeButton = screen.getByText('Subscribe to keep access');
  ...
  it('fires BILLING_TRIAL_UPGRADE_CLICKED when Upgrade Now link clicked during trial (no proPriceId)', async () => {
    const { usePage } = await import('@inertiajs/react');
    vi.mocked(usePage).mockReturnValue({
      props: createMockProps({
        platformTrial: {
    ...
    // "Upgrade now" inline link (no proPriceId path) and "Upgrade Now" card footer link
    const upgradeLinks = screen.getAllByText(/Upgrade [Nn]ow/);
